...
from tf_agents.policies import random_tf_policy, greedy_policy, q_policy
...
train_py_env = FilmEnvironment(config_path=env_name, debug =DEBUG, random_init=True)
eval_py_env = FilmEnvironment(config_path=env_name, debug =DEBUG, random_init=True)
...
my_q_policy = q_policy.QPolicy(train_env.time_step_spec(),
                               train_env.action_spec(),
                               q_net)
...
avg_return = compute_avg_return(train_env, agent.collect_policy, num_eval_episodes)
returns = [avg_return]
...
for _ in range(num_iterations):
  ...
  collect_data(train_env, agent.collect_policy, replay_buffer, 10)
